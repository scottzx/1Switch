...
2. 确认 git remote origin 指向用户自己的仓库（`miaoxworld/openclaw-manager`），**不是** upstream（`miaoxworld/openclaw-manager`）
   ```bash
   git remote -v
   # 如果不对，执行：
   git remote set-url origin https://github.com/miaoxworld/openclaw-manager.git
   ```
...
// turbo
```bash
gh run list --repo miaoxworld/openclaw-manager --limit 3
```
...
// turbo
```bash
gh release view vX.X.X --repo miaoxworld/openclaw-manager
```
